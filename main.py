...
class Node:
  ...
  def find(self, prefix):
    ...
    if prefix == prefix[:pointer+1]:
      return self.children[prefix] # if the child is the word, return the child??
    if prefix[:pointer+1] not in self.children:
      return None # if the child is not in the dict, return None
    else:
      return self.children[prefix[:pointer+1]].find(prefix) # run find on the next child
  ...
  def words(self):
    ...
    output = []
    if self.is_word():
      output.append(self._prefix) # if I am a word, append myself to the output
...
def setup(screen_size):
  ...
  mapping = load_map("data/emnist-balanced-mapping.txt")
